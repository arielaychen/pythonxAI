...
if "model" not in st.session_state: # 初始化AI模型
    st.session_state.model = "gpt-4o-mini" # 如果AI模型不存在,設置預設AI模型
...
with col1:
    ...
    with col2:
        ...
        st.session_state.model = st.selectbox(
            "AI模型",
            ["gpt-4o-mini","gpt-4","gpt-4o-search-preview",],
        )

with col3:
    if st.button("🗑️"): # 在第3列顯示清空按鈕
...
for message in st.session_state.history:
    if message["role"] == "user":
        st.chat_message("user",avatar="🪄") # 顯示使用者的訊息，使用指定的頭像
    else:
...
if prompt: # 如果使用者輸入了訊息
    ...
    response = openai.chat.completions.create(
        ...
        + st.session_state.history ,# 將對話紀錄加入訊息列表
    ) # 使用openai.chat.completions.create取得AI助手回傳的訊息內容

    assistant_message = response.choices[0].message.content # 取得AI助手的訊息內容
