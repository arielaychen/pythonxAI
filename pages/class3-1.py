...
col1.button("按鈕1",key="btn1") # 在col1中建立一個按鈕類似("按鈕1")
col2.button("按鈕2",key="btn2") # 在col2中建立一個按鈕類似("按鈕2")
...
with col1: # 在col1使用 with 語句放更多東西
    if st.button("按鈕1", key="btn8"): # 在col1建立一個按鈕
        st.balloons() # 在col1中建立一個文字 
with col2: # 在col2使用 with 語句放更多東西
    st.button("按鈕2", key="btn9") # 在col2建立一個按鈕
...
for i in range(len(cols)):
    with cols[i]:
        st.button(f"按鈕{i+1}", key=f"btn{i+10}") # 在cols[i]中建立一個按鈕
